...
class XpDrain: Block(FabricBlockSettings.copyOf(Blocks.STONE).requiresTool().strength(1.5F, 6.0F)) {
    ...
    override fun onEntityCollision(state: BlockState, world: World, pos: BlockPos, entity: Entity) {
        if(!world.isClient && entity is PlayerEntity) {
            ...
            i = MathHelper.floor(i*KibeMod.CONFIG.miscellaneousModule.xpDrainSpeedMultiplier)
            ...
            while (i > 0) {
                ...
                if(getTotalExperience(entity) > j) {
                    ...
                    Transaction.openOuter().also {
                        insertable?.insert(FluidVariant.of(FluidCompendium.LIQUID_XP), toInsertAmount, it)?.let { insertedAmount = it }
                    }.commit()
                    if(insertedAmount > 0) {
