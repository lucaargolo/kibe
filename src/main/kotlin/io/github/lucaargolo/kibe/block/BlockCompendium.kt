...
object BlockCompendium : RegistryCompendium<Block>(Registries.BLOCK) {
    ...
    fun <E : FlowableFluid> registerFluidBlock(identifier: Identifier, entry: E): Lazy<FluidBlock> {
        return register(identifier, FluidBlock(entry, Settings.copy(Blocks.LAVA)), hasItem = false)
    }

    override fun <E: Block> register(identifier: Identifier, entry: E, vararg tags: TagKey<Block>): Lazy<E> {
        return register(identifier, entry, *tags, hasItem = true)
    }

    fun <E : Block> register(string: String, entry: E, vararg tags: TagKey<*>, hasItem: Boolean = true): Lazy<E> {
        return register(ModIdentifier.of(string), entry, *tags, hasItem = hasItem)
    }

    @Suppress("UNCHECKED_CAST")
    fun <E : Block> register(identifier: Identifier, entry: E, vararg tags: TagKey<*>, hasItem: Boolean = true): Lazy<E> {
        if(hasItem) {
