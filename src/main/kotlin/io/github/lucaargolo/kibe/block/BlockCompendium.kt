...
object BlockCompendium : RegistryCompendium<Block>(ForgeRegistries.BLOCKS) {
    ...
    val PLACER by register("placer", { Placer(Settings.copy(Blocks.COBBLESTONE).luminance { if(it[Properties.ENABLED]) 15 else 0 }) })
    val BREAKER by register("breaker", { Breaker(Settings.copy(Blocks.COBBLESTONE)) })
    ...
    fun <E : FlowableFluid> registerFluidBlock(string: String, entry: ObjectHolderDelegate<E>): ObjectHolderDelegate<FluidBlock> {
        val blockDelegate = register(string, { FluidBlock(entry, Settings.copy(Blocks.LAVA)) }, false)
