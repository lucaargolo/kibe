...
class Tank: BlockWithEntity(FabricBlockSettings.copyOf(Blocks.GLASS).strength(0.5F).nonOpaque().luminance { state -> state[Properties.LEVEL_15] }.sounds(BlockSoundGroup.GLASS)) {
    ...
    override fun <T : BlockEntity?> getTicker(world: World, blockState: BlockState?, blockEntityType: BlockEntityType<T>?): BlockEntityTicker<T>? {
        return if(!world.isClient) checkType(blockEntityType, BlockEntityCompendium.TANK, TankBlockEntity::tick) else null
    }
    ...
    override fun onUse(state: BlockState, world: World, pos: BlockPos, player: PlayerEntity, hand: Hand, hit: BlockHitResult): ActionResult {
        return (world.getBlockEntity(pos) as? TankBlockEntity)?.let {
            FluidHelper.interactPlayerHand(it.tank, player, hand)
        } ?: ActionResult.FAIL
    }
