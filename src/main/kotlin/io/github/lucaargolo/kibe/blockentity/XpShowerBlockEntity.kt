...
class XpShowerBlockEntity(pos: BlockPos, state: BlockState): BlockEntity(BlockEntityCompendium.XP_SHOWER, pos, state) {
    ...
    companion object {
        ...
        fun tick(world: World, pos: BlockPos, state: BlockState, blockEntity: XpShowerBlockEntity) {
            ...
            i = MathHelper.floor(i* KibeMod.CONFIG.miscellaneousModule.xpShowerSpeedMultiplier)
            ...
            while (i > 0) {
                ...
                Transaction.openOuter().also {
                    extractable?.extract(FluidVariant.of(FluidCompendium.LIQUID_XP), toExtractAmount, it)?.let { extractedVolume = it }
                }.commit()
