...
class ConveyorBelt(private val speed: Double): Block(FabricBlockSettings.copyOf(Blocks.IRON_BLOCK).mapColor(MapColor.IRON_GRAY).requiresTool().strength(5.0F, 6.0F).sounds(BlockSoundGroup.METAL)) {
    ...
    override fun getPlacementState(ctx: ItemPlacementContext): BlockState? {
        return defaultState
            .with(HorizontalConnectingBlock.NORTH, ctx.world.getBlockState(ctx.blockPos.south()).block is io.github.lucaargolo.kibe.blocks.ConveyorBelt)
            .with(HorizontalConnectingBlock.SOUTH, ctx.world.getBlockState(ctx.blockPos.north()).block is io.github.lucaargolo.kibe.blocks.ConveyorBelt)
            .with(HorizontalConnectingBlock.EAST, ctx.world.getBlockState(ctx.blockPos.west()).block is io.github.lucaargolo.kibe.blocks.ConveyorBelt)
            .with(HorizontalConnectingBlock.WEST, ctx.world.getBlockState(ctx.blockPos.east()).block is io.github.lucaargolo.kibe.blocks.ConveyorBelt)
    ...
    override fun getStateForNeighborUpdate(state: BlockState, facing: Direction, neighborState: BlockState, world: WorldAccess, pos: BlockPos, neighborPos: BlockPos): BlockState {
        return if (facing.axis.type == Direction.Type.HORIZONTAL)
            state.with(HorizontalConnectingBlock.NORTH, world.getBlockState(pos.south()).block is io.github.lucaargolo.kibe.blocks.ConveyorBelt)
                 .with(HorizontalConnectingBlock.SOUTH, world.getBlockState(pos.north()).block is io.github.lucaargolo.kibe.blocks.ConveyorBelt)
                 .with(HorizontalConnectingBlock.EAST, world.getBlockState(pos.west()).block is io.github.lucaargolo.kibe.blocks.ConveyorBelt)
                 .with(HorizontalConnectingBlock.WEST, world.getBlockState(pos.east()).block is io.github.lucaargolo.kibe.blocks.ConveyorBelt)
    ...
    override fun getOutlineShape(state: BlockState?, view: BlockView?, pos: BlockPos?, ePos: ShapeContext?): VoxelShape =
        io.github.lucaargolo.kibe.blocks.ConveyorBelt.Companion.SHAPE
    ...
    override fun getCullingShape(state: BlockState?, view: BlockView?, pos: BlockPos?): VoxelShape =
        io.github.lucaargolo.kibe.blocks.ConveyorBelt.Companion.EMPTY
