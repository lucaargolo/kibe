...
class HeatLampBlockEntity(heatLamp: HeatLamp): BlockEntity(getEntityType(heatLamp)), Tickable {
    ...
    override fun setLocation(world: World, pos: BlockPos) {
        ...
        (world as? ServerWorld)?.let {
            setupHeatLamp(it, ChunkPos(pos), this)
        }
    }

    companion object {

        private val globalActiveLamps = linkedMapOf<RegistryKey<World>, LinkedHashMap<ChunkPos, LinkedHashSet<HeatLampBlockEntity>>>()

        fun setupHeatLamp(world: ServerWorld, center: ChunkPos, lamp: HeatLampBlockEntity) {
            val activeLamps = globalActiveLamps.getOrPut(world.registryKey) { linkedMapOf() }
