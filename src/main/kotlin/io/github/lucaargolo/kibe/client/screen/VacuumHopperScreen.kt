...
class VacuumHopperScreen(screenHandler: VacuumHopperScreenHandler, inventory: PlayerInventory, title: Text): HandledScreen<VacuumHopperScreenHandler>(screenHandler, inventory, title) {
    ...
    override fun render(context: DrawContext, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        if(mouseX in (startX+100..startX+112) && mouseY in (startY+18..startY+70)) {
            ...
            context.drawTooltip(textRenderer, listOf(if(tank.isResourceBlank) Text.translatable("tooltip.kibe.empty") else FluidVariantAttributes.getName(tank.variant), Text.literal("${FluidHelper.getMb(stored)} / ${capacity/81} mB").formatted(Formatting.GRAY)), mouseX, mouseY)
        }
    }
